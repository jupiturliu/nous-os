#!/usr/bin/env python3
"""Evidence-backed Heartbeat workflow.

The workflow can use an installed Synapse runtime, but its deterministic local
Adapter remains the default. Mutable files are rooted in ``NOUS_OS_HOME``.
"""
...
from nous_os.core.events import EvidenceEvent, EventStore
...
from nous_os.evaluation.trading import TradingEvaluator, CLS_V2_FIELDS as TRADING_CLS_FIELDS
...
def configure_runtime_paths(paths: RuntimePaths, *, create: bool = True) -> None:
    """Configure legacy internal file operations below one Runtime Home."""
    global RUNTIME_PATHS, RUNTIME_DIR, RUNTIME_AGENT_BUS, RUNTIME_EPISODE_LOGGER
    global RUNTIME_ALERTS, RUNTIME_INSIGHTS, RUNTIME_EPISODES, RUNTIME_EPISODES_SQLITE
    global RUNTIME_DASHBOARD, RUNTIME_RESEARCH_RECORDS
    RUNTIME_PATHS = paths.ensure() if create else paths
    RUNTIME_DIR = paths.state / "heartbeat"
    ...
    RUNTIME_EPISODES = RUNTIME_DIR / "episodes" / "episodes.jsonl"
    RUNTIME_EPISODES_SQLITE = RUNTIME_DIR / "episodes" / "episodes.sqlite"
    RUNTIME_DASHBOARD = paths.projections / "dashboard-data.json"
    RUNTIME_RESEARCH_RECORDS = paths.projections / "research-records"


configure_runtime_paths(RUNTIME_PATHS, create=False)
...
def runtime_backend_policy() -> Dict:
    ...
    return {
        ...
        "sqlite_path": "runtime://state/heartbeat/episodes/episodes.sqlite",
...
def _trading_workspace_root() -> Path:
    configured = os.environ.get("NOUS_TRADING_WORKSPACE")
    return Path(configured).expanduser().resolve() if configured else RUNTIME_PATHS.home / "external"
...
def build_research_record(goal: str, runs: List[Dict], override: Dict, benchmark: Dict, demo_mode: str) -> Dict:
    ...
    return {
        ...
        "snapshot_ref": "projection://dashboard-data",
    }


def prepare_research_record(snapshot: Dict) -> None:
    record = snapshot["research_record"]
    record["artifact_path"] = f"projection://research-records/{record['run_id']}.json"
    record["latest_path"] = "projection://research-records/latest.json"
    snapshot.setdefault("paths", {})["research_record"] = record["artifact_path"]
    snapshot["paths"]["latest_research_record"] = record["latest_path"]
...
def build_dashboard_snapshot(goal: str, runs: List[Dict], override: Dict, demo_mode: str = DEFAULT_DEMO_MODE) -> Dict:
    ...
    return {
        ...
        "paths": {
            "implementation_queue": "runtime://state/heartbeat/agent-bus/implementation_queue.json",
            "learning_queue": "runtime://state/heartbeat/agent-bus/learning_queue.json",
            "alerts": "runtime://state/heartbeat/agent-bus/alerts.json",
            "dashboard_data": "projection://dashboard-data.json",
        },
    }
...
def run_heartbeat_flow(
    goal: str = DEFAULT_GOAL,
    override_kind: str = DEFAULT_OVERRIDE_KIND,
    demo_mode: str = DEFAULT_DEMO_MODE,
    *,
    runtime_home: str | Path | None = None,
    profile: str = "research",
) -> Dict:
    configure_runtime_paths(RuntimePaths.resolve(runtime_home))
    demo_mode = normalize_demo_mode(demo_mode)
    ...
    prepare_research_record(snapshot)
    store = EventStore(RUNTIME_PATHS)
    artifact = store.write_artifact("heartbeat", snapshot, artifact_id=snapshot["research_record"]["run_id"])
    store.append(EvidenceEvent(
        event_type="heartbeat.completed",
        run_id=snapshot["research_record"]["run_id"],
        profile=profile,
        producer="heartbeat",
        payload={
            "demo_mode": demo_mode,
            "cls_v2": snapshot["benchmark"]["cls_v2"]["score"],
            "dashboard_projection": "projections/dashboard-data.json",
        },
